...
func (client *WSClient) init() {
	...
	if client.ConnNum <= 0 {
		client.ConnNum = 1
		logger.Infof("invalid ConnNum, reset to %v", client.ConnNum)
	}
	if client.ConnectInterval <= 0 {
		client.ConnectInterval = 3 * time.Second
		logger.Infof("invalid ConnectInterval, reset to %v", client.ConnectInterval)
	}
	if client.PendingWriteNum <= 0 {
		client.PendingWriteNum = 100
		logger.Infof("invalid PendingWriteNum, reset to %v", client.PendingWriteNum)
	}
	if client.MaxMsgLen <= 0 {
		client.MaxMsgLen = 4096
		logger.Infof("invalid MaxMsgLen, reset to %v", client.MaxMsgLen)
	}
	if client.HandshakeTimeout <= 0 {
		client.HandshakeTimeout = 10 * time.Second
		logger.Infof("invalid HandshakeTimeout, reset to %v", client.HandshakeTimeout)
	}
...
func (client *WSClient) dial() *websocket.Conn {
	for {
		...
		logger.Infof("connect to %v error: %v", client.Addr, err)
