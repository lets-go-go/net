...
func (handler *WSHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Debugf("upgrade error: %v", err)
		return
	}
...
func (server *WSServer) Start() {
	...
	if server.MaxConnNum <= 0 {
		...
		logger.Infof("invalid MaxConnNum, reset to %v", server.MaxConnNum)
	}
	if server.PendingWriteNum <= 0 {
		server.PendingWriteNum = 100
		logger.Infof("invalid PendingWriteNum, reset to %v", server.PendingWriteNum)
	}
	if server.MaxMsgLen <= 0 {
		server.MaxMsgLen = 4096
		logger.Infof("invalid MaxMsgLen, reset to %v", server.MaxMsgLen)
	}
	if server.HTTPTimeout <= 0 {
		server.HTTPTimeout = 10 * time.Second
		logger.Infof("invalid HTTPTimeout, reset to %v", server.HTTPTimeout)
	}
