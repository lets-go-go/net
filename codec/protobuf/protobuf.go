...
func (p *Processor) Register(msg proto.Message) uint16 {
	...
	if msgType == nil || msgType.Kind() != reflect.Ptr {
		logger.Fatal("protobuf message pointer required")
	}
	if _, ok := p.msgID[msgType]; ok {
		logger.Fatalf("message %s is already registered", msgType)
	}
	if len(p.msgInfo) >= math.MaxUint16 {
		logger.Fatalf("too many protobuf messages (max = %v)", math.MaxUint16)
	}
...
func (p *Processor) SetRouter(msg proto.Message, msgRouter *chanrpc.Server) {
	...
	if !ok {
		logger.Fatalf("message %s not registered", msgType)
	}
...
func (p *Processor) SetRawHandler(id uint16, msgRawHandler MsgHandler) {
	if id >= uint16(len(p.msgInfo)) {
		logger.Fatalf("message id %v not registered", id)
	}
